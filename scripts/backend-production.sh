...
Options:
1. Install Gradle, then run: ./scripts/backend-production.sh
2. Add Gradle Wrapper to this repo, then run: ./scripts/backend-production.sh
3. Start with Docker after Docker Desktop is running:
   SPRING_PROFILES_ACTIVE=production docker compose up --build
EOF
