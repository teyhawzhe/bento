...
系統 SHALL 為 `dev`、`staging`、`production` 提供明確且可重現的 build / startup 方式，讓開發者與部署流程可選擇目標環境並套用對應設定。文件 MUST 說明各環境如何指定目標 profile、需要哪些環境變數，以及可用的本機或容器化啟動方式。

#### Scenario: 文件中可找到 dev 的啟動方式
- **WHEN** 開發者查閱專案說明文件
- **THEN** 文件明確描述 `dev` 環境的啟動方式、必要設定與驗證方法

#### Scenario: 文件中可找到 staging 或 production 的設定方式
- **WHEN** 維運人員查閱專案說明文件
- **THEN** 文件明確描述如何套用 `staging` 或 `production` 的環境參數與部署設定

### Requirement: 敏感設定不得寫死於版本庫
系統 SHALL 允許 `staging` 與 `production` 使用外部注入方式提供敏感設定，例如資料庫密碼、SMTP 帳密或正式寄件者資訊。這些敏感值 MUST 不作為固定明文寫入版本庫。

#### Scenario: production 環境使用外部機制提供敏感資訊
- **WHEN** production 環境需要資料庫密碼、JWT secret 或 SMTP 密碼
- **THEN** 系統透過環境變數、部署平台 secret 或等效外部機制注入該值，而非依賴版本庫內固定明文
